...
| `STEP <s0> <s1> <s2> <btn> <key> [hotkey] [n]` | yes | run `n` frames (default 1, max 64) with the same input: `Sliders[0..2]` = turn/pitch/throttle, `btn` = `Buttons` bitmask (bit 0/1 missile, 2 minigun-held, 3 handbrake), `key` = engine keycode for `KbdLastHit/Down` (0 = none), `hotkey` = hotkey-binding id (-1 = none). One state reply after the last frame — frame-skip batching without per-frame socket round-trips |
